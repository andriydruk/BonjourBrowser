...
class ServiceActivity : AppCompatActivity(), ServiceDetailFragment.ServiceDetailListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            val service = IntentCompat.getParcelableExtra(intent, SERVICE, BonjourServiceInfo::class.java)
            serviceDetailFragment = ServiceDetailFragment.newInstance(service!!)
            supportFragmentManager.beginTransaction().replace(R.id.content, serviceDetailFragment).commit()
        } else {
            serviceDetailFragment = supportFragmentManager.findFragmentById(R.id.content) as ServiceDetailFragment
        }
    ...
    override fun onServiceUpdated(service: BonjourServiceInfo) {
        mServiceName.text = service.displayName
        mDomain.text = getString(R.string.domain, service.domain)
        mRegType.text = getString(R.string.reg_type, service.regType)
        mLastTimestamp.text = getString(R.string.last_update, Utils.formatTime(System.currentTimeMillis()))
    }

    override fun onServiceStopped(service: BonjourServiceInfo) {
        setResult(Activity.RESULT_OK, Intent().putExtra(SERVICE, service))
        finish()
    }
    ...
    companion object {
        ...
        fun startActivity(context: Context, service: BonjourServiceInfo) {
        ...
        fun startActivity(context: Context, service: BonjourServiceInfo, isRegistered: Boolean): Intent =
            Intent(context, ServiceActivity::class.java)
                .putExtra(SERVICE, service)
                .putExtra(REGISTERED, isRegistered)
